...
Everything is stored in a PostgreSQL database running as a Docker service alongside the app.
...
Register at `/register` with your full name, email, and password. After registration you are redirected to the login page. JWT token stored in an `httpOnly` cookie. Login is rate-limited to 5 requests/minute per IP.
...
│   │   ├── pages/              # LoginPage, RegisterPage, DashboardPage, JobDetailPage, CVPage, FiltersPage
...
This starts **7 services**: FastAPI (port 8001), PostgreSQL, Redis, Celery Worker, Celery Beat, and the Vite frontend dev server (port 5173).

Open **http://localhost:5173**, go to `/register` to create your account, then log in.
...
| Variable | Default | What it controls |
|---|---|---|
| `DATABASE_URL` | `postgresql://jobradar:jobradar@postgres:5432/jobradar` | PostgreSQL connection string — matches the `postgres` service in Docker Compose |
| `REDIS_URL` | `redis://redis:6379/0` | Redis connection string for Celery — matches the `redis` service name in Docker Compose |
| `CV_UPLOAD_DIR` | `/cv_data` | Where uploaded CV PDFs are stored inside the container |
...
- Registration is open — create your account at `/register` (full name, email, password)
